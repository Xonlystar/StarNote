...
public class ColorPickerDialog extends Dialog {
	...
	public ColorPickerDialog(Context context, String title,
		...
		this(context, Color.TRANSPARENT, title, listener);
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(manager.getDefaultDisplay().getHeight()<manager.getDefaultDisplay().getWidth())
		{
			height = (int) (manager.getDefaultDisplay().getHeight() * 0.8f);
			width = (int) (manager.getDefaultDisplay().getWidth() * 0.4f);
		}
	...
	private class ColorPickerView extends View {
		...
		public ColorPickerView(Context context, int height, int width) {
			...
			//标题参数
			mTitlePaint = new Paint(Paint.LINEAR_TEXT_FLAG);
		...
		protected void onDraw(Canvas canvas) {
			...
			//画标题
			canvas.drawText(title, -22, 8, mTitlePaint);
			//是否显示中心圆外的小圆环
